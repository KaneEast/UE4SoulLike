...
AMainCharacter::AMainCharacter()
{
	...
	CameraBoom->TargetArmLength = 350.f;			// カメラからプレイヤーまでの距離
	CameraBoom->bUsePawnControlRotation = true;		// controllerからの操作を受ける	

	GetCapsuleComponent()->SetCapsuleSize(27.f, 88.f);

	FollowCamera = CreateDefaultSubobject<UCameraComponent>(TEXT("FollowCamera"));
	FollowCamera->SetupAttachment(CameraBoom, USpringArmComponent::SocketName);
	FollowCamera->bUsePawnControlRotation = false;	// カメラを回転するとプレイヤーが移動する時カメラ方向に向く
